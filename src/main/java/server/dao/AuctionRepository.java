...
public class AuctionRepository {
    ...
    /** Lấy 1 auction theo ID --> mở detail screen, placeBid, payment...)*/
    ...
    /** Lấy tất cả auction chưa kết thúc hoàn toàn, loại bỏ PAID, CANCELED -> dùng cho Main view*/
    ...
    /** Lọc theo 1 status -> tab RUNNING/ Admin PENDING SCREEN */
    ...
    /**
     * Lọc nhiều status cùng lúc. -> Dùng cho Main View
     */
    public List<Auction> findByStatus(List<AuctionStatus> statuses) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            System.err.println("[AuctionRepo] findByStatus(List) thất bại.");
            e.printStackTrace();
        }
        return result;
    }

    /** Lấy tất cả auction của Seller --> Dùng cho màn hình của Seller */
    ...
    /** Lấy tất cả auction mà bidder từng tham gia*/
     ...
     */
    /**
     * JOIN row → AuctionCardDTO.
     */
    private AuctionCardDTO mapCardRow(ResultSet rs)
            throws SQLException {
